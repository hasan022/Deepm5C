...
def emb_seq_PseKNC(sequence, num = 25):
    mul_emb_vec = []
    emb_vec = []
    cat_dict = {"A":[1.0,1.0,1.0], "T":[0.0,1.0,0.0], "G":[1.0,0.0,0.0], "C":[0.0,0.0,1.0], "N":[0.0,0.0,0.0]}

    for i in range(len(sequence)):
        p_vec = cat_dict[sequence[i]]
        current_seq = sequence[:i + 1]
        a_vec = [collections.Counter(current_seq)[sequence[i]]/len(current_seq)]
        emb_vec.append(p_vec + a_vec)
        
    emb_vec = np.array(emb_vec)
    mul_emb_vec = np.array([emb_vec[i:i + num, :].reshape([4 * num]) for i in range(len(emb_vec) - num + 1)])
                           
    return  mul_emb_vec

def emb_seq(seq, num = 25):
...
class pv_data_sets(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        emb_mat = emb_seq_PseKNC(self.seq[idx], 25)
...
class burt_process():
    def __init__(self, out_path, tra_batch_size = 128, val_batch_size = 128, features = 100, lr = 0.00001, n_epoch = 10000, early_stop = 20, thresh = 0.5, loss_type = "imbalanced"):
      ...
      opt = optim.Adam(params = net.parameters(), lr = self.lr)
        ...
        with open(self.out_path + "/deep_HV_result.txt", 'w') as f:
            ...
            for epoch in range(self.n_epoch):
                ...
                #train_thresh = thresholds[argmax(fscore)]
...
#parser.add_argument('-vi', '--validation_inpath', help='Path')
parser.add_argument('-o', '--outpath', help='Path')
parser.add_argument('-l', '--losstype', help='Path', default = "imbalanced", choices=["balanced", "imbalanced"])
#parser.add_argument('-m', '--model_path', help='Path')

path = parser.parse_args().path
#val_path = parser.parse_args().validation_inpath
...
for i in range(1, 6):
    ...
    net.pre_training(train_dataset, val_dataset)
